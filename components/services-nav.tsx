...
export function ServicesNav() {
  ...
  return (
    <>
      ...
      <aside className="sticky top-20 hidden h-[calc(100svh-5rem)] w-60 shrink-0 flex-col overflow-y-auto border-r border-hair bg-zemin px-8 pb-8 pt-10 lg:flex xl:w-64">
        {/* Araç — disiplinlerden ayrı durur */}
        <p className="eyebrow text-fog">Stüdyo</p>
          ...
          <Link
            ...
            className={cn(
              ...
              isActive("kompozisyon")
                ? "pl-7 font-medium text-kalem"
                : "pl-0 text-fog hover:pl-2 hover:text-kalem",
            )}
          >
            <span
              aria-hidden
              className={cn(
                "absolute left-0 top-1/2 h-px -translate-y-1/2 bg-kalem transition-[width,opacity] duration-500 ease-[var(--ease-out-expo)]",
        ...
        <div aria-hidden className="seam my-6 text-kalem" />
        <p className="eyebrow text-fog">Hizmetler</p>
        ...
        <div className="mt-auto border-t border-hair pt-6">
          <p className="text-[11px] leading-4 text-fog">On disiplin. Tek başına ya da bütün olarak.</p>
          <a
      ...
      {/* Mobil — yatay şerit */}
      <div className="sticky top-16 z-30 border-b border-hair bg-zemin/90 backdrop-blur-md lg:hidden">
        <nav
        ...
        >
          <Link
            ...
            className={cn(
              ...
              isActive("kompozisyon")
                ? "border-kalem font-medium text-kalem"
          ...
          <Link
            ...
            className={cn(
              "whitespace-nowrap border-b py-4 text-[13px] transition-colors duration-500",
              galeriAktif ? "border-kalem font-medium text-kalem" : "border-transparent text-fog",
            )}
          >
            Galeri
          </Link>
          <span aria-hidden className="my-4 w-px shrink-0 bg-hair" />
          {services.map((s) => {
            const active = isActive(s.slug);
            return (
              <Link
                ...
                className={cn(
                  "whitespace-nowrap border-b py-4 text-[13px] transition-colors duration-500",
                  active ? "border-kalem font-medium text-kalem" : "border-transparent text-fog",
                )}
              >
                {s.short}
              </Link>
