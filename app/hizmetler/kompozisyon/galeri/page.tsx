  ...
  return (
    <div className="flex flex-1 flex-col bg-paper px-6 pb-24 pt-8 md:px-10 md:pt-10 lg:px-12">
      <header className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="eyebrow text-ash">Stüdyo · Galeri</p>
        ...
        <div className="flex items-center gap-6">
          ...
          <Link href="/hizmetler/kompozisyon" className="eyebrow u-line hover:text-ink">
            Yeni kompozisyon
          </Link>
        </div>
      </header>
      ...
      <p className="mt-12 max-w-[62ch] text-[11px] leading-4 text-ash">
...
function Bos({ baslik, metin, eylem }: { baslik: string; metin: string; eylem?: boolean }) {
  return (
    <div className="mt-12 flex flex-1 items-center justify-center border border-dashed border-mist py-24">
      <div className="max-w-[42ch] px-8 text-center">
        <p className="font-display text-xl">{baslik}</p>
        <p className="mt-3 text-sm leading-relaxed text-smoke">{metin}</p>
        {eylem && (
          <Link
            href="/hizmetler/kompozisyon"
            className="mt-6 inline-block eyebrow u-line hover:text-ink"
          >
            Stüdyoya git
          </Link>
        )}
